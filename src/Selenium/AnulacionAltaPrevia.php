...
class AnulacionAltaPrevia extends Operation
{
    ...
    public function doOperation()
    {
        ...
        */
         ...
         */
        $selectBaja = new WebDriverSelect($this->driver->findElement(WebDriverBy::name('cbo_ListaAltasBajas001')));
        $selectBaja->selectByVisibleText('Alta');

        /*
         * Rellenar fecha real de la baja.
         * Tres campos (día, mes, año)
         */
        $this->driver->findElement(WebDriverBy::name('txt_SDFDIAB'))->sendKeys($this->operation->getFra()->format("d"));
        $this->driver->findElement(WebDriverBy::name('txt_SDFMESB'))->sendKeys($this->operation->getFra()->format("m"));
        $this->driver->findElement(WebDriverBy::name('txt_SDFAOB'))->sendKeys($this->operation->getFra()->format("Y"));
